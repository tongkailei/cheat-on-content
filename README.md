...
When upgrading the rubric, **all** calibrated samples with real performance data must be re-scored with the new formula. New ranking must match actual ranking in ≥4/5 samples before the upgrade is allowed.
...
| Learn from account | `学这个账号 @xxx` | Dissect viral patterns |
...
| Published | `已发布` / `发布链接是 X` | Register publish metadata |
...
|-------|---------|----------|
| `cheat-init` | 初始化 / init | Entry: init project skeleton + rubric |
| `cheat-learn-from` | 学这个账号 / learn from | Import benchmark account, dissect patterns |
| `cheat-seed` | 找选题 / seed | Cold-start topic launcher |
| `cheat-score` | 打分这篇 [path] | Single draft scoring (no file written) |
| `cheat-score-blind` | (internal) | Blind sub-agent for contamination-free scoring |
| `cheat-predict` | 预测 / 启动预测 | Blind prediction + immutable log |
| `cheat-shoot` | 拍了 / 录完了 | Register shoot (buffer +1) |
| `cheat-publish` | 已发布 / 发布链接 | Publish metadata registration (buffer -1) |
...
| `cheat-trends` | 抓热点 / 今天有什么可做 | Trend scraping (supplements candidate pool) |
| `cheat-status` | 状态 / 看板 | Status dashboard (incl. buffer alerts) |
...
```
你：初始化
→ Creates project skeleton, default rubric (opinion video)
